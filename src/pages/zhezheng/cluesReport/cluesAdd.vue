  ...
  <div class="clues_warp">
    ...
    <div class="cluesMsg">
      ...
      <div class="form" :class="edit ? '' : 'unclick'">
        ...
        <div class="item">
          <p :class="formData.happenDate ? 'active' : ''">线索时间</p>
          <input
            type="text"
      ...
      <div v-if="clueEventPeopleList.length != 0">
        ...
        >
          <div class="form_people">
            人员{{ index + 1 }}
            <div class="add_btn" @click="personRetrieve(index)">
...
export default {
  ...
  data() {
    return {
      ...
      edit: true,
      tabs: [],
      tabIndex: localStorage.getItem("cluesAddTabIndex") / 1 || 1,
      formData: {
        address: "",
        districtsId: "",
        roadName: "",
  ...
  async created() {
    ...
    console.log("111", that.roadData1);
  ...
  methods: {
    ...
    //   })
    //     .then((res) => {
    //       that.formData.sexId = res.value;
    //       that.formData.sex = res.key;
    //     })
    //     .catch((err) => {});
    // },
    //  选择时间
    ...
    changeIndex(i) {
      const that = this;
      that.tabIndex = i;
      localStorage.setItem("cluesAddTabIndex", i);
      this.$forceUpdate();
    },
    // 移除关联
    remove(i) {
    ...
    validateFn() {
      ...
      //   return false;
      // }
      // if (!that.formData.characterLabelsId) {
      //   that.alert("请输入性格标签");
      // }
      return true;
    },
            ...
            if (res.buttonIndex == 1) {
              ...
              if (data.success) {
                dd.toast({
                  ...
                  .then((res) => {
                    // that.$router.push("/zhezheng/cluesReport/cluesList");
                  })
                  .catch((err) => {});
                that.$router.push("/zhezheng/cluesReport/cluesList");
              }
