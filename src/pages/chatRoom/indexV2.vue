          ...
          <div class="chat_box_style right_chat_box" v-else>
            <div class="text_box">
              <div>
                <div class="text_content_box" v-if="item.content">
                  <!-- {{ !!!item.peopleNames ? item.content : '' }} -->
                  <iframe v-if="!!!item.peopleNames && item.content" src="https://m.amap.com/navi/?dest=116.470098,39.992838&destName=阜通西&hideRouteIcon=1&key=60594691b1a6578205c9cb1eb0b9aee0"></iframe>
                  <div v-if="!!item.peopleNames" class="at_box">
                    <div
                      v-for="(item2, idx2) in optionAtString(item)"
                      :class="item2.type == 2 ? 'at_style' : ''"
...
export default {
  ...
  methods: {
    ...
    async getHistory(that, type) {
      ...
      dd.downloadAudio({
        mediaId: newList[2].fileId
      }).then(res => {
          console.log('downloadAudio', res);
      }).catch(err => {    
        console.log('downloadAudio-err', err);
      })
    ...
    toolChlick(type) {
      ...
      if (type == 5) {
        ...
        dd.searchOnMap({
          ...
          const url = `https://m.amap.com/navi/?dest=${result.longitude},${result.longitude}&destName=阜通西&hideRouteIcon=1&key=60594691b1a6578205c9cb1eb0b9aee0`
          const data = {
            messageType: 3,
            contentType: 1,
            url,
