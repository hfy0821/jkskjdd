<template>
  <div class="event_warp">
    ...
    <div>
      ...
      <!-- <DbFeedback v-if="tabIndex == 5" :missionDetail="missionDetail" /> -->
      <HandlePs
        v-if="tabIndex == 5"
  ...
  methods: {
    ...
    async getDetail() {
      ...
      const res = await _http(
        "socialGovernance/monitorMissionMobile/getMonitorDetil",
        params,
        "get"
      );
      // const res = await getMonitorDetail(params);
      if (res.success) {
        that.detailData = res.data.eventDetail || {};
        that.emergencyDetail = res.data.emergencyDetail || {};
        that.missionDetail = res.data.missionDetail || {};
        that.dutyDetail = res.data.dutyDetail || {};
        that.baseInstructionVOList = res.data.baseInstructionVOList || [];
      }
    },
    changeIndex(i) {
      const that = this;
