<template>
  <div class="health_warp">
    ...
    <div class="subTab" v-show="tabIndex===1">
      <!-- '','','','' -->
      <span
        :class="subTabIndex===1?'active':''"
        @click="checkSubTab(0)"
        v-show="right.includes('healthEvent:handleManagement')"
      >主办</span>
      <span
        :class="subTabIndex===2?'active':''"
        @click="checkSubTab(1)"
        v-show="right.includes('healthEvent:handleManagement')"
      >协办</span>
      <span
        :class="subTabIndex===3?'active':''"
        @click="checkSubTab(2)"
        v-show="right.includes('healthEvent:eventManagement')"
      >已发</span>
      <span
        :class="subTabIndex===4?'active':''"
        @click="checkSubTab(3)"
        v-show="right.includes('healthEvent:eventManagement')"
      >已回复</span>
    ...
    <div class="report" @click="add" v-if="right.includes('healthEvent:add')">
...
import { getEventListByTodoForAPP, getRestOfMenu } from "../../api/a";
export default {
  name: "health",
  components: {
    Nav
  ...
  data() {
    return {
      ...
      tabIndex: localStorage.getItem("healthTabIndex") / 1 || 1,
      subTabIndex: localStorage.getItem("healthSubIndex") / 1 || 1,
      list: [],
      total: 0,
      params: {
        status: 1,
  ...
  async created() {
    ...
    //   }
    // });
    const { data } = await getRestOfMenu({
      parentId: "healthManage0101",
      screen: 0
    });
    if (data && data.length) {
      this.right = data.map(v => v.name);
    }
    this.tabs[2].hide = !this.right.includes(
      "healthEvent:distributionManagement"
    );
    if (!this.right.includes("healthEvent:handleManagement")) {
      if (this.right.includes("healthEvent:eventManagement")) {
        this.tabIndex = localStorage.getItem("healthTabIndex") / 1 || 1;
        this.subTabIndex = localStorage.getItem("healthSubIndex") / 1 || 3;
      } else {
        this.tabIndex = localStorage.getItem("healthTabIndex") / 1 || 2;
      }
    }
    this.getData();
  },
  methods: {
    ...
    changeIndex(i) {
      console.log(i);
      this.tabIndex = i;
      localStorage.setItem("healthTabIndex", i);
      this.pageSize = 10;
      this.keyword = "";
      this.getData();
    },
    checkSubTab(i) {
      ...
      localStorage.setItem("healthSubIndex", i + 1);
      this.pageSize = 10;
      this.keyword = "";
      this.getData();
    },
    ...
    toDetail(id) {
      this.$router.push(`health/detail?id=${id}&type=${this.params.type}`);
    },
    add() {
      this.$router.push("health/add");
    }
  }
};
</script>
