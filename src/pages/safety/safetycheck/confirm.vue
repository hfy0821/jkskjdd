<template>
  <div class="confirm_warp">
    ...
    <div class="btn">
      ...
      <span @click="handClick(2)" v-if="i == '99'">申请办结</span>
    </div>
  </div>
</template>
<script>
...
export default {
  data() {
    return {
      ...
      formData: {
        ...
        copyUserName: [],
        rectifyObj: [],
        checkPerson: JSON.parse(localStorage.getItem("userInfo")).name,
        rectifyContent: "",
        checkDate: "",
      },
      files: [],
  ...
  async created() {
    console.log(ding.mobBackUrl,'6666')
  ...
  methods: {
    handClick(i) {
      if (i == 2) {
        this.$router.push(
          `/safety/safetycheck/confirm2?i=${i}&id=${this.id}&handleType=${this.$route.query.handleType}&handleDepartmentId=${this.$route.query.handleDepartmentId}`
        );
      }
    },
    // 提交
    confirm() {
      ...
      that.formData.dingLinkUrl =
        ding.mobBackUrl+"/fyDingtalkWeb/index.html#/listing/3";
      dd.device.notification.confirm({
        message: "确定指派？",
        title: "提示",
        buttonLabels: ["取消", "确定"],
    ...
    selectTime() {
      ...
      dd.showDatePicker({
        ...
        value: that.formData.cutOffTime, //默认显示日期
      })
        .then((result) => {
