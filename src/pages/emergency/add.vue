<template>
  <div class="reportedAdd_warp multiSelectClass mapClass">
    ...
    <div class="form" :class="edit?'':'unclick'">
      ...
      <div class="item">
        <p :class="formData.eventName&&formData.eventName.length?'active':''">事件类别</p>
        <div>
          <input
...
import { upload, addEvent2,addHandle, getHandleEventDetailByIdForApp } from "../../api/a";
export default {
  name: "reportedAdd",
  components: {
    multiSelect,
  ...
  async created() {
    ...
    if (this.id) {
      ...
      if (data.success) {
        ...
        this.formData.sourceType = 115;
        this.formData.isTeamWork = 1;
        this.formData.authName = "emergencyHandleManagement:edit";
        this.formData.parentAuthName =
          "emergencyEvent:emergencyHandleManagement";
  ...
  methods: {
    ...
    handClick(i) {
      ...
      this.formData.handType = 0;
      this.formData.teamWorkType = 2;
      let objCopy = JSON.parse(JSON.stringify(this.formData));
      this.formData.handleEvent = objCopy;
      const that = this;
          ...
          if (res.buttonIndex) {
            dd.device.notification.showPreloader({
              ...
              async onSuccess(result) {
                const data = await addHandle(that.formData);
